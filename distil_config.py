...
class DistilConfig(TrainingArguments):
    ...
    # CaOPD (Calibration-Aware On-Policy Distillation) parameters
    use_empirical_calibration: bool = field(
        default=False,
        metadata={
            "help": "If True, enable CaOPD: group rollouts by prompt, compute empirical P_acc from K "
            "rollouts, replace confidence in student completion and teacher prompt with P_acc. TIS is disabled "
            "for this path since the training trajectory is modified."
        },
    )
