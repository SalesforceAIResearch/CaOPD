...
# ============================================================================
# CaOPD: answer verification and confidence parsing utilities
# ============================================================================

def _verify_answer_impl(completion_text: str, ground_truth_str: str) -> bool:
    """Verify a single completion against ground truth (JSON string).

    Supports two formats:
    - Chemistry MCQ: ground_truth is JSON list of [{"Answer": "A"}].
    - Tool-use: ground_truth is JSON list of [{"Action": "...", "Action_Input": "..."}].
    """
    try:
        golden_answer = json.loads(ground_truth_str)
    except (json.JSONDecodeError, TypeError):
        return False
    if not golden_answer or not isinstance(golden_answer, list):
        return False
    first = golden_answer[0]
    if not isinstance(first, dict):
        return False

    # Chemistry MCQ
    if "Answer" in first:
        expected = str(first.get("Answer", "")).strip().upper()
        if not expected:
            return False
        _answer_tag = re.compile(r"<answer>\s*([^<]*?)\s*</answer>", re.DOTALL | re.IGNORECASE)
        matches = _answer_tag.findall(completion_text)
        if not matches:
            return False
        return matches[-1].strip().upper() == expected

    # Tool-use
    try:
        from eval.tooluse_correctness import check_correctness_one
    except ImportError:
        _script_dir = Path(__file__).resolve().parent
        _eval_dir = _script_dir / "eval"
        if _eval_dir.is_dir() and (_eval_dir / "tooluse_correctness.py").is_file():
            import sys
            if str(_script_dir) not in sys.path:
                sys.path.insert(0, str(_script_dir))
            from eval.tooluse_correctness import check_correctness_one
        else:
            raise ImportError("eval.tooluse_correctness not found for verify_answer")
    correct, _ = check_correctness_one(completion_text, golden_answer)
    return correct


def verify_answer(completion_text: str, ground_truth: str) -> float:
    """CaOPD wrapper: verify one completion against ground truth. Returns 1.0 or 0.0."""
    return float(_verify_answer_impl(completion_text, ground_truth))


def _parse_confidence_from_completion(completion_text: str) -> Optional[float]:
    """Extract numeric confidence from completion (e.g. 'Confidence: 0.75')."""
    match = re.search(r"Confidence:\s*(\d+(?:\.\d+)?)", completion_text, re.IGNORECASE)
    if match:
        try:
...
class DistilTrainer(BaseTrainer):
    ...
    def _generate_and_score_completions(
        ...
        # ---- CaOPD: group rollouts, compute P_acc, replace confidence ----
        use_calibration = getattr(self.args, "use_empirical_calibration", False)
        if use_calibration and mode == "train" and len(inputs) > 0 and "ground_truth" in inputs[0]:
            completions_text = [
            ...
            for i in range(len(prompts)):
                key = json.dumps(prompts[i], sort_keys=True)
                prompt_to_indices[key].append(i)

            _conf_fmt_re = re.compile(r"Confidence:\s*\d+(?:\.\d+)?", re.IGNORECASE)
            n_with_conf = sum(1 for t in completions_text if _conf_fmt_re.search(t))
            conf_adherence = n_with_conf / len(completions_text) if completions_text else 0.0
            self._metrics[mode].setdefault("confidence/format_adherence", []).append(conf_adherence)

            forced_completion_texts = [None] * len(completions_text)
            dynamic_teacher_prompts = [None] * len(teacher_prompts)

            # Anchor used in both demo and correct_solution teacher templates
            teacher_anchor_demo = "Now answer with a response of your own, including the thinking process."
            teacher_anchor_correct = "Correctly solve the original question."

            for _key, indices in prompt_to_indices.items():
                P_acc = sum(verify_answer(completions_text[j], ground_truths[j]) for j in indices) / float(len(indices))

                tag_suffix = f"\nConfidence: {P_acc:.2f}"
                max_comp = max(1, (self.max_completion_length or 256) - 1)
                _strip_inline = re.compile(r"\n?Confidence:.*$", re.IGNORECASE | re.DOTALL)

                for j in indices:
                    # (1) Replace confidence in student completion
                    base = _strip_inline.sub("", completions_text[j]).rstrip()
                    forced = base + tag_suffix
                    while (
                        len(self.processing_class.encode(forced, add_special_tokens=False)) > max_comp
                        and len(base) > 0
                    ):
                        base = base[: max(0, len(base) - 80)]
                        forced = base + tag_suffix
                    forced_completion_texts[j] = forced

                # (2) Replace confidence in teacher demo + insert Note
                _demo_conf_re = re.compile(
                    r"(This is an example.*?Confidence:\s*)[\d.]+",
                    re.DOTALL,
                )
                note_paragraph = (
                    f"Note: The empirical confidence for this question is {P_acc:.2f}. "
                    f"Output this exact value in the 'Confidence:' field.\n\n"
                )
                for j in indices:
                    msg_list = list(teacher_prompts[j])
                    if msg_list and isinstance(msg_list[0], dict) and "content" in msg_list[0]:
                        old_content = msg_list[0]["content"]
                        new_content = _demo_conf_re.sub(rf"\g<1>{P_acc:.2f}", old_content, count=1)
                        # Insert Note before whichever teacher anchor is present
                        for anchor in [teacher_anchor_demo, teacher_anchor_correct]:
                            if anchor in new_content:
                                new_content = new_content.replace(anchor, note_paragraph + anchor)
                                break
                        dynamic_teacher_prompts[j] = [{"role": "user", "content": new_content}]
                    else:
                        dynamic_teacher_prompts[j] = teacher_prompts[j]

            teacher_prompts = [dynamic_teacher_prompts[i] for i in range(len(teacher_prompts))]

            # Re-tokenize forced completions, append EOS
            _max_comp_len = self.max_completion_length or 256
            new_completion_ids_list = []
            for i in range(len(forced_completion_texts)):
                ids = self.processing_class.encode(
                    forced_completion_texts[i],
                    add_special_tokens=False,
                    truncation=True,
                    max_length=_max_comp_len - 1,
                )
                ids = ids + [self.eos_token_id]
                new_completion_ids_list.append(ids)
            completion_ids_list = new_completion_ids_list
            sampling_per_token_logps_list = None  # TIS invalid after trajectory modification

            emp_conf_values = [verify_answer(completions_text[j], ground_truths[j]) for j in range(len(completions_text))]
            if emp_conf_values:
                unique_p_accs = []
                for _key, indices in prompt_to_indices.items():
                    P_acc = sum(verify_answer(completions_text[j], ground_truths[j]) for j in indices) / float(len(indices))
                    unique_p_accs.append(P_acc)
                if unique_p_accs:
                    self._metrics[mode].setdefault("empirical_confidence_mean", []).append(
                        sum(unique_p_accs) / len(unique_p_accs)
                    )
        # ---- End CaOPD ----
        ...
        with torch.no_grad():
            ...
            # Skip when CaOPD (use_empirical_calibration) is active: trajectory was modified, vLLM logps are invalid.
            _caopd_active = getattr(self.args, "use_empirical_calibration", False)
            generate_every = self.args.steps_per_generation * self.num_iterations  # generation frequency
            if not self.generate_from_teacher and not _caopd_active and (
